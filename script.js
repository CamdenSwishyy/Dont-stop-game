...
function isAILeaderboardName(name) { return /gemini|google\s*ai|chat\s*gpt|openai|copilot/i.test(name || ''); }
function renderLeaderboardRows(players) {
  const list = $('leaderboard-list');
  list.innerHTML = players.filter(item => !isAILeaderboardName(item.player_name || item.name)).map((item, index) => { const score = item[boardMode] || 0; const safeName = escapeHtml(item.player_name || item.name); const initials = escapeHtml(safeName.slice(0, 2).toUpperCase()); const isYou = item.player_name === accounts.find(account => account.email === currentEmail)?.name; return `<li class="${isYou ? 'you' : ''}"><span class="rank ${index === 0 ? 'rank-gold' : index === 1 ? 'rank-silver' : index === 2 ? 'rank-bronze' : ''}">${String(index + 1).padStart(2, '0')}</span><span class="leader-avatar ${isYou ? 'you-avatar' : index % 2 ? 'blue' : 'orange'}">${initials}</span><span class="leader-info"><strong>${safeName}${isYou ? ' · you' : ''}</strong><small>${boardMode.toUpperCase()} ${score.toLocaleString()}</small></span><b>${score.toLocaleString()}</b></li>`; }).join('');
}
