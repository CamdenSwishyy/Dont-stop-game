...
function render() {
  ...
  $('shop-button').textContent = state.shopOwned ? 'OWNED' : '20,000 ✦'; $('shop-button').disabled = state.shopOwned;
  const xpUpgradeCost = 500 * (state.xpBoost + 1); const coinUpgradeCost = 650 * (state.coinBoost + 1); const cooldownCosts = [20000, 50000, 100000]; const luckUpgradeCost = 200 + state.luckLevel * 150; const recoveryCosts = [30000, 90000, 180000];
  ...
  $('recovery-upgrade-button').textContent = state.recoveryLevel >= 3 ? 'MAX' : `${recoveryCosts[state.recoveryLevel]} ✦`; $('recovery-upgrade-button').disabled = state.recoveryLevel >= 3;
...
$('shop-button').addEventListener('click', () => { if (state.coins < 20000) { showToast('NOT ENOUGH COINS.', false); return; } state.coins -= 20000; state.shopOwned = true; localStorage.setItem(upgradeKey, JSON.stringify({ xpBoost: state.xpBoost, coinBoost: state.coinBoost, cooldownLevel: state.cooldownLevel, shopOwned: state.shopOwned })); showToast('STATIC BLOOM UNLOCKED · +25% REWARDS'); render(); });
$('xp-upgrade-button').addEventListener('click', () => buyUpgrade('xpBoost', 500, 'XP BOOST')); $('coin-upgrade-button').addEventListener('click', () => buyUpgrade('coinBoost', 650, 'COIN BOOST')); $('cooldown-upgrade-button').addEventListener('click', () => buyUpgrade('cooldownLevel', 0, 'COOLDOWN', [20000, 50000, 100000])); $('luck-upgrade-button').addEventListener('click', () => buyUpgrade('luckLevel', 200, 'LUCK BOOST', null, 20, 150)); $('recovery-upgrade-button').addEventListener('click', () => buyUpgrade('recoveryLevel', 0, 'RECOVERY CORE', [30000, 90000, 180000]));
