...
function claimDailyLogin() { if (savedLogin.date === today) return; loginStreak = getNextLoginStreak(); const reward = Math.min(250 + loginStreak * 50, 1000); state.coins += reward; savedLogin.date = today; savedLogin.streak = loginStreak; savedLogin.reward = reward; localStorage.setItem(loginKey, JSON.stringify(savedLogin)); showToast(`DAY ${loginStreak} LOGIN REWARD · +${reward} COINS`); render(); }
function render() {
